...
The zero-one filter in the block diagram below is a super-simple low pass filter. It works by forking the signal into two, delaying one of the forks by one sample and then adding them back together. The name is nothing to do with binary or what values it outputs by the way, it's to do with how you would map its responce in the z-plane, which is much beyond the scope of this article. 
...
Assuming the sin's amplitude is 1, the x values are going to jump right from 1 to -1 to 1 to -1. You can see that this is the fastest oscilation we can represent here (and therefor the highest frequency). If you delay it by one sample you get the opposite sequence, as we can see:
